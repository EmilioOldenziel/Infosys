...
for k = 2:numel(frequentItems)
    for j = 1:size(frequentItems{k},1)
        ...
        for i = 1:numel(frequentItems{k}(j,:))-1
            consequencelist = nchoosek(frequentItems{k}(j,:),i);
            skiplist = [];
            for l = 1:size(consequencelist,1)
                % check if is superset of a skip
                skip = false;
                for t = 1:size(skiplist, 1) % for each skip
                    bools = ismember(consequencelist(l,:), skiplist(t,:));
                ...
                if ~skip
                    antecedent = find(~ismember(frequentItems{k}(j,:),consequencelist(l,:)));
                    confidence = support{k}(j)*ntrans/sum(all(dataset(:,frequentItems{k}(j,antecedent)),2));
                    if confidence >= minconf
                        sup(end+1) = support{k}(j);
                        conf(end+1) = confidence;
                        consequencestr = '';
                        for m = 1:size(consequencelist,2)
                            consequencestr = [consequencestr,trlbl{consequencelist(l,m)},','];
                        end
                        antecedentstr = '';
                        for m = 1:numel(antecedent)
                            antecedentstr = [antecedentstr,trlbl{frequentItems{k}(j,antecedent(m))},','];
                        end
                        AR{end+1} = [antecedentstr(1:end-1),' -> ',consequencestr(1:end-1),sprintf(' [%2.2f,%2.2f]',sup(end),conf(end))];
                    else
                       % minimum confidence not met, store consequence to skip
                      skiplist = [skiplist; consequencelist(l,:)];
                   end
                end
            end
        end        
